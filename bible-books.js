// Canon metadata: [id, englishName, simplifiedChineseName, chapterCount, englishAliases]
export const BOOKS = [
...
// Folder names in the lguenth/mdbible repo, in canon order (same order as BOOKS).
...
export const BY_ID = {};
BOOKS.forEach(b => { BY_ID[b[0]] = { id: b[0], name: b[1], zh: b[2], chapters: b[3], aliases: b[4] }; });
export const ORDER = BOOKS.map(b => b[0]);

// Editorial section headings + paragraph groupings written for this app.
// Both translations use the same verse ranges, which is what makes the two
// columns line up row for row. Chapters without an entry fall back to
// even paragraph chunks.
